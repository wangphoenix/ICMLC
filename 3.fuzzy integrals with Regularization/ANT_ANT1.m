function [trainScoreList,testScoreList,maxTrainUList,maxTrainEList] = ANT_ANT(data)
    %K交叉验证中参数
    K=10;
...
function [accuarcy]=fitfun(U,e,Z,trueResult)
    ...
    %选择个很平衡
    %accuarcy=0.5*fitnessResult(1)+0.1*fitnessResult(3)+0.4*fitnessResult(3)
    accuarcy=1*fitnessResult(2)
