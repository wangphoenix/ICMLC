...
%     [index]=crossvalind('Kfold',row,K);
    ...
    for k=1:K
        ...
        %使用L1/2计算出一组u，保存在U中
        [U,e,lambdas]=useL12(Z,train(:,col));
        %使用fisher分类分析准确度
...
function [U,e,lambdas]=useL12(Z,trueResult)
     %使用L1/2计算出u，result中的beta就是u值,Z矩阵和真实值，result.beta就是对应u值
     [result]=L12_L12fit(Z,trueResult);
     U=result.beta;
     e=result.beta0;
