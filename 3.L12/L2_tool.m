function [maxTrainScoreList,testScoreList,maxTrainUList,maxTrainEList,trainZero] = L2_tool(data )
...
%   此处显示详细说明
    ...
    for k=1:K
        ...
        %使用L1计算出一组u，保存在U中
        [U,e,lambdas]=useL2_tool(Z,train(:,end));
        %使用fisher分类分析准确度
...
function [U,e,lambdas] = useL2_tool(Z,trueResult)
    % options for solver
    options.verbose=1;
