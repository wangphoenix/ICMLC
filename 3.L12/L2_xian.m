function [maxTrainScoreList,testScoreList,maxTrainUList,maxTrainEList] = L2_xian(data )
...
%   此处显示详细说明
    ...
    for k=1:K
        ...
        %使用L1计算出一组u，保存在U中
        [U,e,lambdas]=useL2_xian(Z,train(:,end));
        %使用fisher分类分析准确度
...
function [U,e,lambdas] = useL2_xian(Z,trueResult)
    [row,col]=size(Z)
